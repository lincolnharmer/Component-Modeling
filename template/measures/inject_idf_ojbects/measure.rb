...
class InjectIDFOjbects < OpenStudio::Ruleset::WorkspaceUserScript
  ...
  def arguments(workspace)
	...
	source_idf_path.setDefaultValue('C:/Users/Public/Documents/template/run/000_CreateBaselineFromPrototypeBuilding/scheduledLoads.ems')
  ...
  def run(workspace, runner, user_arguments)
	...
	objects = workspace.getObjectsByType('OtherEquipment'.to_IddObjectType)
	# DO STUFF
if not objects.empty?

  objects.each do |object|
  if object.name.to_s == "Heating Load 1"
    puts 'Setting schedule for : #{object.getString(0)}'
    object.setString(3, "HeatingLoad")
  else
    puts 'Setting schedule for : #{object.getString(0)}'
    object.setString(3, "CoolingLoad")	
